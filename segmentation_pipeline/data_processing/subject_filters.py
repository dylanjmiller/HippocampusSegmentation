...
class RequireREAttributes(SubjectFilter):
    """ Filters subject attributes based on regex

    Args:
        attributes: Defines the attributes for subjects.

            - A dictionary that defines required RE values for keys in the subject dictionary.
              Note that the keys themselves are also required. There is an additional
              constraint that the values matches the RE.
    """
    def __init__(
            self,
    ...
    def subject_filter(self, subject):
        ...
        matches = []
        for attrib_name, attrib_value in self.attributes.items():
            if attrib_name not in subject:
                return False

            match = re.match(attrib_value, subject.get(attrib_name))
            if match == None:
                return False
